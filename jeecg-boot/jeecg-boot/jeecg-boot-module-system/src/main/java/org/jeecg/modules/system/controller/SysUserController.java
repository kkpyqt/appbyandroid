...
//                json.put("avatar", item.getSysUser().getAvatar());
	...
	public Result<JSONObject> userRegister(@RequestBody JSONObject jsonObject, SysUser user) {
		...
		try {
			...
			user.setEmail(email);
			user.setPhone(phone);
			user.setOrgCode("A01A03");
			user.setStatus(CommonConstant.USER_UNFREEZE);
			user.setDelFlag(CommonConstant.DEL_FLAG_0);
			user.setActivitiSync(CommonConstant.ACT_SYNC_0);
			sysUserService.addUserWithRole(user,"ee8626f80f7c2619917b6236f3a7f02b");//默认临时角色 test

            //登录模式，校验用户有效性  2021.9.21
            SysUser sysUser = sysUserService.getUserByPhone(user.getPhone());
//            注册时，自动增加子用户1，2
            HmUserSetList  userSetList =new HmUserSetList();
//            user.setId(String.valueOf(Calendar.getInstance().getTimeInMillis()));
            userSetList.setUserId(sysUser.getId());

            userSetList.setSetId("1");
            log.info(userSetList.getUserId()+','+userSetList.getId());

            iHmUserSetListService.save(userSetList)  ;
//            HmUserSetList  userSetList2 =new HmUserSetList();  //直接只能增加1分组
//            userSetList2.setSetId("2");
//            userSetList2.setUserId(sysUser.getId());
////            user.setId(String.valueOf(Calendar.getInstance().getTimeInMillis()));
//            log.info(userSetList.getUserId()+','+userSetList2.getId());
//            iHmUserSetListService.save(userSetList)  ;
			result.success("注册成功");
		} catch (Exception e) {
